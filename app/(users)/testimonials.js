  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[13px] font-black text-gray-800 leading-tight">{t.name}</p>
      ...
      <div className="flex items-center gap-2.5">
        <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${t.avatarBg} flex items-center justify-center text-white text-[11px] font-black shrink-0`}>
          {t.initials}
        </div>
        <div className="min-w-0">
          <p className="text-[11px] font-bold text-gray-700 leading-tight truncate">{t.product}</p>
          <p className="text-[12px] font-black text-pink-500 mt-0.5">{t.price}</p>
        </div>
      </div>
    </div>
  );
...
export default function Testimonials() {
  return (
    <section className="py-20 bg-gradient-to-br from-[#fdf8f2] via-white to-[#fde8ef] overflow-visible relative">
      <div
        className="absolute inset-0 opacity-[0.03] pointer-events-none"
        style={{
          backgroundImage: "radial-gradient(circle, #f2608a 1.5px, transparent 1.5px)",
      ...
      <div className="relative z-10">
        ...
        <div className="text-center mb-12 px-4">
          <span className="inline-block bg-pink-100 text-pink-500 text-xs font-black tracking-widest uppercase px-4 py-1.5 rounded-full mb-3">
            Reviews
          </span>
          <h2 className="text-3xl md:text-4xl font-black text-gray-800">
            What Parents <span className="text-pink-500">Are Saying</span>
          </h2>
          <p className="text-gray-400 text-sm mt-2 font-semibold">
            Real reviews from real families who love our toys
          </p>
        </div>
