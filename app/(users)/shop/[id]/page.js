...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setProduct(data.toy);
        setSelectedImage(data.toy?.images?.[0] || "https://placehold.co/800x600?text=No+Image");
  ...
  const imageList = useMemo(() => {
    if (!product?.images?.length) return ["https://placehold.co/800x600?text=No+Image"];
...
// Replace your addToCart function with this minimal version:
const addToCart = () => {
  if (!product) return;
  ...
  try {
    ...
    const existingIndex = existingItems.findIndex((item) => item.id === productId);

    if (existingIndex >= 0) {
      const currentQty = Number(existingItems[existingIndex].quantity) || 0;
      const nextQty = Math.min(currentQty + finalQuantity, stock);
      existingItems[existingIndex].quantity = nextQty;
    } else {
      existingItems.push({
        id: productId,
        quantity: finalQuantity
        // No name, no image - just ID and quantity
      });
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-4 py-16 flex justify-center items-start">
        <div className="max-w-md w-full bg-white rounded-2xl border border-red-100 p-8 text-center shadow-sm">
          ...
          <Link href="/shop" className="mt-6 inline-flex items-center justify-center rounded-xl bg-blue-600 px-6 py-3 text-base font-medium text-white shadow-sm transition-all hover:bg-blue-700 w-full">
            Back to Shop
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <nav aria-label="Breadcrumb" className="mb-8">
          <Link href="/shop" className="inline-flex items-center text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors">
  ...
  height={1000}
            ...
            <div className="flex flex-col">
              <div className="mb-6">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">{product.title}</h1>
                <div className="mt-4 flex items-center">
                   <p className="text-3xl font-black text-blue-600">₹{product.price !== undefined ? parseFloat(product.price).toFixed(2) : '0.00'}</p>
                </div>
                <p className="mt-4 text-lg text-gray-600 leading-relaxed">{product.description || "No description available for this toy."}</p>
              </div>

              <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-2 text-sm">
                <div className="rounded-xl bg-gray-50 p-4 border border-gray-100 text-center sm:text-left">
                  <dt className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-semibold">Category</dt>
                  <dd className="font-bold text-gray-900">{product.category || "-"}</dd>
                </div>
                <div className="rounded-xl bg-gray-50 p-4 border border-gray-100 text-center sm:text-left">
                  <dt className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-semibold">Brand</dt>
                  <dd className="font-bold text-gray-900">{product.brand || "-"}</dd>
                </div>
                <div className="rounded-xl bg-gray-50 p-4 border border-gray-100 text-center sm:text-left">
                  <dt className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-semibold">Gender</dt>
                  <dd className="font-bold text-gray-900">{product.gender || "-"}</dd>
                </div>
                <div className="rounded-xl bg-gray-50 p-4 border border-gray-100 text-center sm:text-left">
                  <dt className="text-gray-500 mb-1 text-xs uppercase tracking-wider font-semibold">Age Group</dt>
                  <dd className="font-bold text-gray-900">{product.age || "-"}</dd>
                </div>
              </div>

              {product.tags && product.tags.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-sm font-medium text-gray-900 mb-3 block">Tags</h3>
              ...
              <div className="mt-auto border-t border-gray-200 pt-8">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                  <div className="flex items-center rounded-xl border border-gray-300 bg-white flex-shrink-0 h-14 w-full sm:w-auto mt-1 sm:mt-0">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const nextValue = Math.min(Math.max(Number(e.target.value) || 1, 1), stock);
                    ...
                    <button 
                      ...
                      disabled={Number(product.stock) <= 0 || quantity >= product.stock}
                    >
                      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <button
                    ...
                    onClick={addToCart}
                    disabled={Number(product.stock) <= 0}
                    className="flex-1 flex items-center justify-center gap-2 h-14 rounded-xl bg-blue-600 px-8 text-lg font-bold text-white shadow-sm transition-all hover:bg-blue-700 hover:shadow-md disabled:cursor-not-allowed disabled:bg-gray-400"
                  >
                    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {cartMessage && (
                  <div className={`mt-4 rounded-lg p-4 text-sm font-bold ${cartMessage.includes("Added") ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}>
                    {cartMessage}
                  </div>
                )}

                <div className="mt-8 flex flex-col sm:flex-row gap-4 border-t border-gray-200 pt-8">
                   <button
                    type="button"
                  ...
                  <Link href="/cart" className="flex items-center justify-center sm:w-1/3 px-6 h-14 rounded-xl border-2 border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors">
                    View Cart
                  </Link>
                </div>
              </div>
