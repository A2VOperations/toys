"use client";
import React, { useState } from "react";
import Link from "next/link";
import styles from "./footer.module.css";

const waNumber = (process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "923001234567").replace(/\D/g, "");

const WaIcon = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);
const FbIcon = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
);
const IgIcon = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
  </svg>
);

const PRIVACY = `We collect only what's needed to process your orders and improve your experience. Your personal data — name, address, contact details — is stored securely and never sold to third parties. WhatsApp numbers are used solely for order updates. You may request deletion at any time.`;
const TERMS   = `By placing an order you agree to our pricing and delivery terms. All products are subject to availability. Kiddex reserves the right to refuse service. Payments are processed securely. Disputes are governed by local law. We are not liable for courier-caused delays.`;

const RESOURCES = [
  { label: "Home",    href: "/" },
  { label: "About",   href: "/about" },
  { label: "Shop",    href: "/shop" },
  { label: "Contact", href: "/contact" },
];
const STORE_INFO = ["Best Sellers", "New Arrivals", "Flash Sale", "Top Rated", "Gift Sets"];

const STARS = [
  { top: "12%", left: "22%",  right: undefined, delay: "0s"   },
  { top: "30%", left: "48%",  right: undefined, delay: "0.8s" },
  { top: "55%", left: "35%",  right: undefined, delay: "1.5s" },
  { top: "18%", left: undefined, right: "12%",  delay: "0.4s" },
  { top: "65%", left: undefined, right: "28%",  delay: "1.1s" },
];

function TooltipItem({ label, title, text }) {
  return (
    <li>
      <div className={styles.tooltipWrap}>
        <button className={styles.tooltipBtn}>
          {label}
        </button>
        <div className={styles.tooltipContent}>
          <div className={styles.tooltipTitle}>
            {title}
          </div>
          <div className={styles.tooltipText}>{text}</div>
        </div>
      </div>
    </li>
  );
}

export default function Footer() {
  const [name, setName] = useState("");

  const scrollTop = () => window.scrollTo({ top: 0, behavior: "smooth" });

  const openWA = () => {
    const msg = encodeURIComponent(
      `Hi! My name is ${name || "a customer"} and I'm interested in your toys 🧸`
    );
    window.open(`https://wa.me/${waNumber}?text=${msg}`, "_blank");
  };

  return (
    <footer className={styles.footer}>
     
      {/* ── Wavy top border ── */}
      <div className={styles.waveContainer}>
        <svg viewBox="0 0 1440 40" preserveAspectRatio="none" className={styles.waveSvg}>
          <path d="M0,20 C120,40 240,0 360,20 C480,40 600,0 720,20 C840,40 960,0 1080,20 C1200,40 1320,0 1440,20 L1440,0 L0,0 Z" fill="#fce4ef"/>
        </svg>
      </div>

      {/* ── Main body ── */}
      <div className={styles.main}>

        {/* Decorative stars */}
        {STARS.map((s, i) => (
          <span
            key={i}
            className={styles.star}
            style={{
              top: s.top,
              left: s.left,
              right: s.right,
              animation: `twinkle 3s ease-in-out ${s.delay} infinite`,
            }}
          >★</span>
        ))}

        {/* ── Grid ── */}
        <div className={styles.grid}>

          {/* ── Brand ── */}
          <div className={styles.brand}>
            <Link href="/" className={styles.logo}>
              <span>K</span>
              <span>I</span>
              <span>D</span>
              <span>D</span>
              <span>E</span>
              <span>X</span>
            </Link>
            <div className={styles.infoText}>
              <p>57 Heol Isaf Station Road</p>
              <p>Cardiff, Pakistan</p>
              <p>info@kiddex.com</p>
            </div>
            <div className={styles.socials}>
              <a href={`https://wa.me/${waNumber}`} target="_blank" rel="noreferrer"
                 className={`${styles.socialIcon} ${styles.iconWa}`}>
                <WaIcon />
              </a>
              <a href="https://facebook.com" target="_blank" rel="noreferrer"
                 className={`${styles.socialIcon} ${styles.iconFb}`}>
                <FbIcon />
              </a>
              <a href="https://instagram.com" target="_blank" rel="noreferrer"
                 className={`${styles.socialIcon} ${styles.iconIg}`}>
                <IgIcon />
              </a>
            </div>
          </div>

          {/* ── Resources ── */}
          <div>
            <p className={styles.sectionTitle}>Resources</p>
            <ul className={styles.list}>
              {RESOURCES.map(({ label, href }) => (
                <li key={label}>
                  <Link href={href} className={styles.link}>
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* ── Support ── */}
          <div>
            <p className={styles.sectionTitle}>Support</p>
            <ul className={styles.list}>
              <TooltipItem label="Privacy Policy"     title="🔒 Privacy Policy"     text={PRIVACY} />
              <TooltipItem label="Terms & Conditions" title="📋 Terms & Conditions" text={TERMS}   />
            </ul>
          </div>

          {/* ── Store Info ── */}
          <div>
            <p className={styles.sectionTitle}>Store Info</p>
            <ul className={styles.list}>
              {STORE_INFO.map((item) => (
                <li key={item}>
                  <Link href="/shop" className={styles.link}>
                    {item}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* ── WhatsApp Input ── */}
          <div className={styles.waSection}>
            <p className={styles.sectionTitle}>WhatsApp Us</p>
            <p className={styles.waDesc}>
              Drop your name and we&apos;ll reach out via WhatsApp with updates &amp; exclusive deals!
            </p>
            <div className={styles.inputWrap}>
              <input
                type="text"
                placeholder="Enter Name"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className={styles.inputBox}
              />
              <button onClick={openWA} className={styles.submitBtn} style={{ cursor: "pointer" }}>
                <WaIcon /> Send
              </button>
            </div>
            <p className={styles.waNote}>
              ⚡ We reply within minutes · 9 AM – 10 PM
            </p>
          </div>

        </div>
      </div>

      {/* ── Bottom bar ── */}
      <div className={styles.bottomBar}>
        <div className={styles.bottomInner}>
          <p className={styles.copyright}>
            Copyright © 2025 <a href="/">Kiddex</a>, Inc. All Rights Reserved
          </p>
          <button onClick={scrollTop} className={styles.scrollTopBtn}>
            ↑
          </button>
        </div>
      </div>

    </footer>
  );
}