...
const ANNOUNCEMENTS = [
  ...
  { icon: "🎁", text: "Gift Wrapping available on all orders — Surprise someone today!" },
...
const MEGA = {
  ...
  tags: ["🔥 Bestseller", "✨ New", "💰 Sale", "🏆 Award Winning", "🎯 Limited Edition"],
  genders: [
  ...
  return (
    ...
    >
      <style>{`
        ...
        .ann-in    { animation: ann-in 0.38s cubic-bezier(.4,0,.2,1) both; }
        .mega-open { animation: nb-fadeDown 0.25s cubic-bezier(.4,0,.2,1) both; }
        .nb-font   { font-family: 'Nunito', sans-serif; }
      `}</style>

      {/* Slide */}
      <div className="flex items-center justify-center h-[36px] px-4 overflow-hidden">
        <div
          key={current}
          className="ann-in flex items-center gap-2 text-white font-bold tracking-wide text-[0.72rem] md:text-[0.78rem] text-center whitespace-nowrap"
        >
...
export default function Navbar({ cartCount = 0, onCartClick }) {
  ...
  const openMega = () => {
    if (window.innerWidth >= 768) { clearTimeout(timerRef.current); setMegaOpen(true); }
  };
  const closeMega = () => {
    if (window.innerWidth >= 768) { timerRef.current = setTimeout(() => setMegaOpen(false), 150); }
  };
  ...
  return (
    <>
      ...
      {/* ── Sticky Nav ── */}
      <nav
        className="nb-font sticky top-0 z-[1000] bg-white border-b-2 border-[#fce4ef] px-5 shadow-[0_4px_20px_rgba(232,67,147,0.08)] relative"
        style={{ fontFamily: "'Nunito', sans-serif" }}
      >
        <div className="max-w-[1280px] mx-auto flex items-center justify-between gap-4 h-[68px]">

          {/* Logo */}
          <Link href="/" className="text-[2rem] font-black leading-none tracking-[-2px] no-underline flex items-center transition-transform duration-200 hover:scale-105">
            ...
            {navLinks.map((link) =>
              link.name === "Shop" ? (
                <li key="Shop">
                  ...
                  >
                    Shop
                    <svg className={`w-[14px] h-[14px] transition-transform duration-300 ${megaOpen ? "rotate-180" : ""}`}
        ...
        <div
          ...
          onMouseLeave={closeMega}
        >
          <div className={`bg-white border-t-[3px] border-t-[#e84393] border-b-2 border-b-[#fce4ef] shadow-[0_24px_80px_rgba(232,67,147,0.13)] transition-all duration-[280ms]
            ${megaOpen ? "opacity-100 visible translate-y-0 pointer-events-auto mega-open" : "opacity-0 invisible -translate-y-3"}`}>
            <div className="max-w-[1280px] mx-auto grid min-h-[360px]" style={{ gridTemplateColumns: "1fr 1fr 1fr 320px" }}>
              ...
              <div className="p-9 border-r border-[#fce4ef]">
                ...
                <ul className="list-none p-0 m-0 flex flex-col gap-1.5">
                  {MEGA.categories.map((c) => (
                    <li key={c.label}>
                      <Link href={`/shop?category=${encodeURIComponent(c.label)}`} className="text-[0.84rem] font-bold text-[#555] no-underline flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all duration-200 hover:bg-[#fff0f6] hover:text-[#e84393] hover:pl-4 group">
              ...
              <div className="p-9 border-r border-[#fce4ef]">
                ...
                <ul className="list-none p-0 m-0 flex flex-col gap-1.5">
                  {["New Arrivals", "Best Sellers", "Flash Sale", "Gift Sets", "Under Rs.1000"].map((l) => {
                    let href = "/shop";
                    if (l === "New Arrivals") href = "/shop?sortBy=latest";
                    else if (l === "Best Sellers") href = "/shop?tags=Bestseller";
                    else if (l === "Flash Sale") href = "/shop?tags=Sale";
                    else if (l === "Under Rs.1000") href = "/shop?maxPrice=1000";
                    return (
                      <li key={l}>
                        <Link href={href} className="text-[0.84rem] font-bold text-[#555] no-underline flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all duration-200 hover:bg-[#fff0f6] hover:text-[#e84393] hover:pl-4 group">
                          <span className="w-2 h-2 rounded-full bg-[#fce4ef] transition-all duration-200 group-hover:bg-[#e84393] group-hover:scale-125" />
                          {l}
                        </Link>
                      </li>
              ...
              <div className="p-10 flex flex-col justify-between"
                style={{ background: "linear-gradient(160deg, #fff0f6 0%, #fce4ef 60%, #ffd6eb 100%)" }}>
                <div>
                  ...
                  <h3 className="text-[2rem] font-black leading-tight text-[#222] mb-3">
                    DISCOVER <span style={{ color: "#e84393" }}>PLAYTIME</span> MAGIC
                  </h3>
                  <p className="text-[0.8rem] text-[#888] leading-relaxed font-semibold mb-6">
                    Curated premium toys for every child. Spark imagination and create memories.
                  </p>
                  <div className="flex gap-4 mb-6">
                    <div>
                      <div className="text-[1.3rem] font-black text-[#e84393]">500+</div>
                      <div className="text-[0.65rem] text-[#aaa] font-bold uppercase">Toys</div>
                    </div>
                    <div>
                      <div className="text-[1.3rem] font-black text-[#e84393]">4.9★</div>
                      <div className="text-[0.65rem] text-[#aaa] font-bold uppercase">Rating</div>
                    </div>
                  </div>
                </div>
                <Link href="/shop"
      ...
      >
        ...
        <div className="p-5 border-b-2 border-[#fce4ef] flex justify-between items-center">
          <span className="text-[1.5rem] font-black tracking-[-1px] flex items-center">
            <span style={{ color: "#e84393" }}>K</span><span style={{ color: "#ff6b35" }}>I</span>
            <span style={{ color: "#ffd700" }}>D</span><span style={{ color: "#4caf50" }}>D</span>
            <span style={{ color: "#2196f3" }}>E</span><span style={{ color: "#9c27b0" }}>X</span>
          </span>
          <button onClick={closeMobileMenu} className="bg-none border-none text-2xl cursor-pointer text-[#e84393] font-black px-3 py-1 rounded-lg hover:bg-[#fce4ef] transition-all">✕</button>
        </div>
          ...
          <div>
            ...
            {mobileShopSubmenu && (
              <div className="ml-4 mt-2 mb-2">
                <div className="mb-6">
                  <div className="text-[0.7rem] font-black tracking-widest uppercase text-[#e84393] mb-3">🗂️ Categories</div>
                  {MEGA.categories.map((c) => (
                    <Link key={c.label} href={`/shop?category=${encodeURIComponent(c.label)}`} onClick={closeMobileMenu}
                ...
                <div className="mb-6">
                  <div className="text-[0.7rem] font-black tracking-widest uppercase text-[#e84393] mb-3">🏷️ Trending Tags</div>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {MEGA.tags.map((t) => {
                      const tagValue = t.replace(/[^a-zA-Z\s]/g, '').trim();
                      return (
                        <Link key={t} href={`/shop?tags=${encodeURIComponent(tagValue)}`} onClick={closeMobileMenu}
                          className="bg-[#fff0f6] border border-[#fce4ef] rounded-full px-3 py-1.5 text-[0.7rem] font-bold text-[#e84393] no-underline">{t}</Link>
                      );
                    })}
                  </div>
                </div>
                <div className="mb-6">
                  <div className="text-[0.7rem] font-black tracking-widest uppercase text-[#e84393] mb-3">👶 Shop by Gender</div>
                ...
                <div className="mb-6">
                  <div className="text-[0.7rem] font-black tracking-widest uppercase text-[#e84393] mb-3">⚡ Quick Access</div>
                  {["New Arrivals", "Best Sellers", "Flash Sale", "Gift Sets", "Under Rs.1000"].map((l) => {
                    let href = "/shop";
                    if (l === "New Arrivals") href = "/shop?sortBy=latest";
                    else if (l === "Best Sellers") href = "/shop?tags=Bestseller";
                    ...
                    return (
                      <Link key={l} href={href} onClick={closeMobileMenu}
                        className="block py-2.5 pl-4 text-[#666] font-semibold text-[0.9rem] no-underline border-b border-[#fce4ef] hover:text-[#e84393]">{l}</Link>
                    );
                  })}
                </div>
              </div>
        ...
        {/* Feature promo */}
        <div className="mx-5 my-2 p-5 rounded-2xl" style={{ background: "linear-gradient(160deg, #fff0f6 0%, #fce4ef 100%)" }}>
          <span className="text-[10px] font-black uppercase text-[#e84393]">✨ The Toy Stack</span>
          <h3 className="text-[1.2rem] font-black mt-1 mb-3 text-[#222]">DISCOVER PLAYTIME MAGIC</h3>
          <div className="flex gap-4 mb-3">
            <div><span className="text-[1.1rem] font-black text-[#e84393]">500+</span><div className="text-[10px] text-[#aaa]">Toys</div></div>
            <div><span className="text-[1.1rem] font-black text-[#e84393]">4.9★</span><div className="text-[10px] text-[#aaa]">Rating</div></div>
          </div>
        ...
        {/* Cart */}
        <Link href="/cart" onClick={(e) => { if (onCartClick) onCartClick(e); closeMobileMenu(); }}
